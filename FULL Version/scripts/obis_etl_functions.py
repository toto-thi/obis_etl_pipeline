...
def _load_and_deduplicate_raw_data(raw_data_path: str) -> pl.DataFrame | None:
    ...
    try:
        ...
        return None # Or raise e? Returning None for now
...
# Optional, usually filter null of some data attribute will be done by data scientist based on their interested.
# def _filter_critical_nulls(df: pl.DataFrame, critical_cols: list[str]) -> pl.DataFrame: # Now accepts critical_cols
#     """Filters rows with nulls in the specified essential columns."""
#     logging.info(f"Helper: Filtering rows with critical nulls in columns: {critical_cols}...")
#     if df is None or df.height == 0:
#          logging.warning("Input DataFrame for null filtering is empty.")
#          return df
...
#         logging.warning(f"Columns specified for null filtering not found in DataFrame: {missing_critical_cols}. Will filter only on existing columns.")

#     df_final = df 
#     if not existing_critical_cols:
#         logging.error("None of the specified critical columns exist for null filtering! Returning DataFrame as is.")
#     else:
#          logging.debug(f"Applying drop_nulls on existing critical columns: {existing_critical_cols}")
#          df_final = df.drop_nulls(subset=existing_critical_cols) 
#          rows_dropped = initial_count - df_final.height
#          if rows_dropped > 0:
#               logging.warning(f"Dropped {rows_dropped} rows due to nulls in critical columns: {existing_critical_cols}")
#          else:
#               logging.info("No rows dropped by null filter.")
...
def transform_and_split_data(raw_data_path: str) -> dict[str, pl.DataFrame] | None:
    ...
    try:
        ...
        # Optional: Final Null Filtering *on Facts Only*
        # critical_fact_cols = ["occurrence_id", "obis_id", "aphia_id", "dataset_id", 
        #                       "latitude", "longitude", "depth_m", "event_date"]
        # df_facts_final = _filter_critical_nulls(df_facts, critical_cols=critical_fact_cols)
